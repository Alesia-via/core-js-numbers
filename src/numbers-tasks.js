...
function getAverage(value1, value2) {
  if (value1 > 0 && value2 > 0 && value1 + value2 > Number.MAX_VALUE) {
    return Number.MAX_VALUE;
  }
  if (value1 < 0 && value2 < 0 && value1 + value2 < Number.MIN_VALUE) {
...
function getSumOfDigits(num) {
  ...
  const sumOfDigits = arr.reduce((sum, current) => sum + Number(current), 0);
  return sumOfDigits;
}
...
function toPrecision(number, precision) {
  const fixedNum = number.toPrecision(precision);
